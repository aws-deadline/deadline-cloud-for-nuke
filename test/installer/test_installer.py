...
def installer_path():
    path = "DeadlineCloudForNukeSubmitter-{platform}-installer.{ext}"
...
def _validate_files(installation_path: Path) -> None:
    ...
    # Check that we have dependencies directly in the installation directory
...
def test_default_location(installer_path: Path):
    ...
    --prefix <prefix>                           Installation Directory
                                                Default: /home/<user>/DeadlineCloudForNukeSubmitter
    """
    # GIVEN
    default_install_location = Path("~/DeadlineCloudForNukeSubmitter").expanduser()
    ...
    def _verify_windows_least_privilege(self, installation_path: Path):
        ...
        for path in [installation_path, *installation_path.rglob("*")]:
            ...
            if _is_admin():
                ...
                bad_perms[path].append(
                    f"Expected to be owned by '{win32security.LookupAccountSid('', user_sid)}' but got '{win32security.LookupAccountSid('', owner_sid)}'"
                )

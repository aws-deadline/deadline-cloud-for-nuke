...
class _SessionState:
    ...
    don't show it again until Nuke is restarted."""
...
def _check_for_update() -> UpdateCheckResult:
    """Check if a newer version of the Nuke submitter is available.
    ...
    return safe_check_for_updates(
        integration_name="deadline-cloud-for-nuke",
...
def check_and_show_update_dialog() -> bool:
    ...
    If the user previously dismissed the dialog in this Nuke session,
    ...
    if update_result.status == UpdateCheckStatus.SUCCESS and update_result.update_available:
        update_dialog = UpdateAvailableDialog(
            ...
            release_notes_url="https://github.com/aws-deadline/deadline-cloud-for-nuke/releases",
        )
        update_dialog.exec_()
        if update_dialog.user_downloaded:
            return True
        # User dismissed — suppress for the rest of this Nuke session

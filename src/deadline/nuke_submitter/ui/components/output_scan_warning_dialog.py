...
try:
    ...
    from PySide6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QCheckBox, QPushButton, QSizePolicy, QFrame
except ImportError:
    # For Nuke 13-15
...
class OutputScanWarningDialog(QDialog):
    ...
    def _setup_ui(self, parsing_outcome: AssetReferencesParsingOutcome):
        ...
        if parsing_outcome.high_level_exception is None:
            ...
            details_message = '\n'.join(traceback.format_exception(parsing_outcome.high_level_exception))

        
        warning_label = QLabel(warning_message)
        warning_label.setWordWrap(True)
        warning_label.setTextInteractionFlags(Qt.TextSelectableByMouse | Qt.TextSelectableByKeyboard)
        ...
        details_label.setTextInteractionFlags(Qt.TextSelectableByMouse | Qt.TextSelectableByKeyboard)
        ...
        def toggle_details_visibility():
            ...
            disclosure_button.setText("▼ hide more details" if self._show_details else "▶ show more details")
            self.adjustSize()

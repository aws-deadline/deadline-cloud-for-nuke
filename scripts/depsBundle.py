...
# _awscrt.cpython-<tag>-<platform>.so while 3.11 gets _awscrt.abi3.so. Resolving it only in
# the base environment would ship whichever the build host produced, so any Nuke whose
# embedded interpreter that single artifact does not cover would fail to import awscrt and
# AWS Console sign-in would break there.
...
def _copy_native_to_base_env(base_env: Path, native_dependency_paths: list[Path]) -> None:
    ...
    collision is a no-op. ``awscrt`` mixes the schemes: Python 3.9 and 3.10 get
    version-specific wheels whose names cannot collide, while every abi3 wheel installs
